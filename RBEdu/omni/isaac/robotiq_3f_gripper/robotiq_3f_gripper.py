...
class Robotiq3F(ParallelGripper):
    ...
    def __init__(
        ...
        self._joint_dof_indicies = np.array([
            None, None,
            None, None, None,
            None, None, None,
            None, None, None
    ...
    def initialize(
        ...
        ParallelGripper.initialize(self, 
        ...
        for index, dof_name in enumerate(dof_names):
            for i, joint_name in enumerate(self._joint_prim_names):
                if joint_name == dof_name:
    ...
    def set_joint_positions(self, positions: np.ndarray) -> None:
        ...
        self._set_joint_positions_func(
            positions=positions, joint_indices=[
                ...
                self._joint_dof_indicies[4],
                self._joint_dof_indicies[5],
                self._joint_dof_indicies[6],
                self._joint_dof_indicies[7],
                self._joint_dof_indicies[8],
            ]
        )
        return
    ...
    def get_joint_positions(self) -> np.ndarray:
        ...
        Returns:
            np.ndarray: joint positions of the left finger joint and the right finger joint respectively.
        """
        return self._get_joint_positions_func(joint_indices=[
            self._joint_dof_indicies[0], 
            self._joint_dof_indicies[1],
            self._joint_dof_indicies[2],
            self._joint_dof_indicies[3],
    ...
    def forward(self, action: str) -> ArticulationAction:
        ...
        if action == "open":
            ...
            if self._action_deltas is None:
                ...
                current_finger_positions = [0.0] * len(self._joint_dof_indicies)
                for i in range(11):
                    current_finger_positions[i] = current_joint_positions[self._joint_dof_indicies[i]]
                for i in range(11):
                    target_joint_positions[self._joint_dof_indicies[i]] = (
                        current_finger_positions[i] + self._action_deltas[i]
                    )
        elif action == "close":
            target_joint_positions = [None] * self._articulation_num_dofs
            if self._action_deltas is None:
