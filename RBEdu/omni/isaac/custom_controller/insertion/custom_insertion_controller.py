...
class CustomInsertionController(BaseController):
    """
    ...
    - Phase 6: Gripper downward to placing position, but with placing offset
    - Phase 7: Insert Object
    - Phase 8: Loose Grip
    - Phase 9: Gripper Back, Keep height 
    ...
    def __init__(
        ...
        if self._robot_type == "franka":
            ...
            if self._events_dt is None:
                self._events_dt = [
                    0.002, 0.007, 0.2, 0.2, 0.01, 0.005, 0.002, 
                    0.05, 0.008, 0.01
                ]
            ...
            if self._events_dt is None:
                self._events_dt = [
                    0.01, 0.01, 0.2, 0.01, 0.01, 0.005, 0.008, 
            ...
            if self._events_dt is None:
                self._events_dt = [
                    0.003, 0.007, 0.2, 0.2, 0.01, 0.005, 0.002, 
                    0.05, 0.008, 0.01
                ]
    ...
    def forward(
        ...
        print(f"[InsertionController] self._event: {self._event}")
        ...
        if self._event == 2:
            ...
            if self._event == 0:
                ...
                position_target = np.array(
                    [
                        ...
                        target_height + end_effector_offset[2] + placing_offset[2],
                    ]
                )
            elif self._event == 6:
                position_target = np.array(
            ...
            if self._event <= 5 or self._event == 10:
    ...
    def _get_target_hs(self, target_height):
        if self._event == 0:
            ...
            h = self._combine_convex(target_height, self._h0, self._mix_sin(self._t))
            # h = self._h0
        else:
            raise ValueError()
        return h
