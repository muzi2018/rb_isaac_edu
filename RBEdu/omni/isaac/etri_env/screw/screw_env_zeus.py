...
class MultiScrewEnvZeus(MultiScrewEnv):
    def __init__(
            self,
            world=None,
        ...
        self._gripper_to_nut_offset = np.array([-0.01, 0.0, 0.13])
        # self._gripper_to_bolt_offset = np.array([-0.001, -0.003, 0.15])
        self._gripper_to_bolt_offset = np.array([-0.00, -0.004, 0.145])
        self._top_of_bolt = (
            np.array([0.0, 0.0, self._bolt_length + (self._nut_height / 2)]) + self._gripper_to_bolt_offset
        )

        if self._robot_type == "2f85":
            self.ROBOT_PATH = self._server_root + "/Projects/ETRI/Manipulator/Zeus/zeus_2f85_screw_d455.usd"
        elif self._robot_type == "3f":
            self.ROBOT_PATH = self._server_root + "/Projects/ETRI/Manipulator/Zeus/zeus_3f.usd"
    ...
    def add_light(self):
        ...
        sphereLight.CreateIntensityAttr(200000)
    ...
    def setup_dataset(self):
        if self._robot_type == "2f85":
            self._robo_data_collector = RoboDataCollector(file_name=f"zeus_2f_screw_env{self._index}_" + self._now_str)
        elif self._robot_type == "3f":
            self._robo_data_collector = RoboDataCollector(file_name=f"zeus_3f_screw_env{self._index}_" + self._now_str)
    ...
    def setup_robots(self):
        ...
        # TODO: Check this for other types of robots
        
        # kps = np.array([
        #     6000000.0, 600000.0, 6000000.0, 600000.0, 25000.0, 15000.0, 
        #     25000.0, 25000.0, 15000.0, 15000.0, 15000.0, 15000.0
        # ])
        # kds = np.array([
        #     600000.0, 60000.0, 300000.0, 30000.0, 3000.0, 3000.0, 
        #     3000.0, 3000.0, 6000.0, 6000.0, 6000.0, 6000.0
        # ])
        ...
        # For Green Rigid Body prim
        # Err - authoring to an instance proxy is not allowed
        # robot_left_finger = self._world.stage.GetPrimAtPath(
        #     prim_path + "/_f85_basic/left_inner_finger_pad/collisions/mesh_0"
        # )
        ...
        #     prim_path + "/_f85_basic/right_inner_finger_pad/collisions/mesh_0"
        # )
        ...
        for i, robot in enumerate(self._robots):
            ...
            if self._controllers[i]._i < min(self._num_nuts, self._num_bolts):
                ...
                self._controllers[i].forward(
                    ...
                    x_offset=0.0,
                    y_offset=self._gripper_to_bolt_offset[1],
                )
