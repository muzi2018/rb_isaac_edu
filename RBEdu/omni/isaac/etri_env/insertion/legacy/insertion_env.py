...
class MultiInsertionEnv(MultiEnv):
    def __init__(
            self,
            world=None,
        ...
        self.USB_A_PATH = self._server_root + "/Projects/ETRI/Connector/USB_A_M_Rev2.usd"
        self.DESKTOP_PATH = self._server_root + "/Projects/ETRI/Env/Insertion/Desktop_Rev3.usd"
...
############################
    ...
    def add_USB_A(self):
        # USB A
        prim_path = f"/World/Env_{self._index}/USB_A_1_{self._index}"
        add_object(
            usd_path=self.USB_A_PATH,
            prim_path=prim_path,
            translate=(0.4, 0.0, 0.64),
            orient=(0.7071, 0.0, -0.7071, 0.0),
            scale=(1.0, 1.0, 1.0),
        )
        add_semantic_data(prim_path, "usb_a")
        return

    def add_Desktop(self):
        # Desktop
        prim_path = f"/World/Env_{self._index}/Desktop_1_{self._index}"
        add_object(
            usd_path=self.DESKTOP_PATH,
            prim_path=prim_path,
            translate=(0.65, -0.1, 0.64),
            orient=(0.0, 0.0, 0.0, 1.0),
            scale=(1.0, 1.0, 1.0),
        )
        add_semantic_data(prim_path, "desktop")
        return
    ...
    def add_USB_A_property(self):
        prim_path = f"/World/Env_{self._index}/USB_A_1_{self._index}"
        self.usb_a_geom, self.usb_a_rigid = add_physical_properties(
            prim_path=prim_path,
            rigid_body=True, 
            collision_approximation="sdf" 
        )
        self._world.scene.add(self.usb_a_geom)
    
    def add_Desktop_property(self):
        prim_path = f"/World/Env_{self._index}/Desktop_1_{self._index}"
        desktop_geom, _ = add_physical_properties(
            prim_path=prim_path,
            rigid_body=False,  
            collision_approximation="sdf" 
        )
        self._world.scene.add(desktop_geom)
